...
data class DeviceRegistrySnapshot(val devices: List<AppleDevice>, val providerErrors: Map<String, String>) {
    ...
    val hasPhysicalAvailable: Boolean get() = physicalAvailableCount > 0
    ...
    val offlineSimulators: List<AppleDevice> get() = simulatorDevices.filter { it.state == DeviceState.OFFLINE }
    ...
    val hasRunnableDevices: Boolean get() = availableDevices.isNotEmpty()
    val runnableDeviceIds: List<String> get() = availableDevices.map(AppleDevice::id)
