...
class VPhoneDeviceOperations(
    ...
    override fun install(deviceId: String, app: Path) =
        when {
            !validId(deviceId) -> DeviceOperationResult(false, "Device id is required")
    ...
    override fun launch(deviceId: String, bundleId: String) = if (!validId(deviceId) || bundleId.isBlank() || bundleId != bundleId.trim()) DeviceOperationResult(false, "Device id and bundle id are required") else execute("launch", deviceId, bundleId)
    override fun logs(deviceId: String, bundleId: String?): Sequence<String> {
        val result = if (!validId(deviceId)) DeviceOperationResult(false, "Device id is required") else execute("logs", deviceId)
    ...
    override fun screenshot(deviceId: String, destination: Path): DeviceOperationResult {
        if (!validId(deviceId) || destination.toString().isBlank()) return DeviceOperationResult(false, "Device id and destination are required")
        runCatching { destination.parent?.let(java.nio.file.Files::createDirectories) }
            .onFailure { return DeviceOperationResult(false, "Unable to prepare screenshot destination: ${it.message ?: "I/O failure"}") }
        return execute("screenshot", deviceId, destination.toString())
    }
