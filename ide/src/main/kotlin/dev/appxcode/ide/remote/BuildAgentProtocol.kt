...
data class BuildAgentRequest(
    ...
    fun isValid(): Boolean = isKnownOperation
...
data class BuildAgentResponse(
    ...
    companion object {
        ...
        fun failed(request: BuildAgentRequest, errorCode: String = BuildAgentErrorCode.BUILD_FAILED, message: String = "operation failed"): BuildAgentResponse =
            rejected(request, errorCode, message)
    }
}
...
data class BuildAgentEndpoint(
    ...
    val displayName: String get() = buildString {
        ...
        pairingId?.let { append(" (").append(it).append(')') }
    }
}
...
data class BuildAgentArtifactRequest(
    ...
    fun missingReferences(available: Collection<String>): List<String> = references.filterNot(available::contains)
